...
func(m *Memory)Put(_ context.Context,s Site)error{
	var err error
	s,err=ValidateAndNormalise(s);if err!=nil{return err}
	m.mu.Lock();defer m.mu.Unlock()
	for _,h:=range s.Hosts{
		if owner,ok:=m.hosts[h];ok&&owner!=s.ID{return ErrHostConflict}
	}
	if old,ok:=m.sites[s.ID];ok{for _,h:=range old.Hosts{delete(m.hosts,normaliseHost(h))}}
	for _,h:=range s.Hosts{m.hosts[h]=s.ID}
	m.sites[s.ID]=s
	return nil
}
...
func(m *Memory)Delete(_ context.Context,id string)error{m.mu.Lock();defer m.mu.Unlock();if s,ok:=m.sites[id];ok{for _,h:=range s.Hosts{if m.hosts[normaliseHost(h)]==id{delete(m.hosts,normaliseHost(h))}};delete(m.sites,id)};return nil}
