...
func main(){
	...
	flag.Parse()
	if *id==""||*code==""{panic("-id and -code are required")}
	...
	if success!=1{panic(fmt.Sprintf("successful redemptions=%d, want exactly 1",success))}
	fmt.Println("PASS: exactly one OTP redemption succeeded")
}
